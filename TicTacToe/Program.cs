...
class Program
{
    ...
    static void GetFinalGrid(string[,] grid, int turn)
    {
        ...
        do
        {
            ...
            i++;
        } while (i <= Constants.GRID_SIZE * Constants.GRID_SIZE && countReplacement < (Constants.GRID_SIZE*Constants.GRID_SIZE - turn));
    }
    ...
    static void Main(string[] args)
    {
        ...
        do
        {
            ...
            {
                //User turn
                UIMethods.DisplayGrid(grid, Constants.GRID_SIZE, Constants.USER_MARK,Constants.COMPUTER_MARK);

                string userChoice = UIMethods.GetUserChoice(grid, Constants.WRONG_CHOICE);
                if (userChoice == Constants.WRONG_CHOICE)
                {
                    continue;
                }

                ReplaceValue(grid, Constants.USER_MARK, userChoice);
            }
            else
            {
                // Computer turn
                
                string computerChoice = FindBestMove(grid, Constants.COMPUTER_MARK); //Value that makes the computer win
                ...
                {
                    ReplaceValue(grid, Constants.COMPUTER_MARK,FindBestMove(grid, Constants.USER_MARK));//Value that can block the user
                }
           ...
           if (globalCheckResult != Constants.WRONG_CHOICE && turn <Constants.GRID_SIZE*Constants.GRID_SIZE)
           {
            ...
            if ( globalCheckResult!= Constants.WRONG_CHOICE || turn == Constants.GRID_SIZE * Constants.GRID_SIZE)
            {
                ...
                {
                    UIMethods.EndGameMessage(grid, "Congratulations, You Won!", Constants.GRID_SIZE,Constants.COMPUTER_MARK,Constants.USER_MARK);
                } 
                if (globalCheckResult == Constants.COMPUTER_MARK)
                {
                    UIMethods.EndGameMessage(grid, "GAME OVER. Try Again.", Constants.GRID_SIZE,Constants.COMPUTER_MARK,Constants.USER_MARK);
                }
                if (turn == Constants.GRID_SIZE * Constants.GRID_SIZE)
                {
                    UIMethods.EndGameMessage(grid, "Draw Game", Constants.GRID_SIZE,Constants.COMPUTER_MARK,Constants.USER_MARK);
                }
            }
